...
public class MetaController {
    ...
    public String meta(Model model) {
        ...
        List<MetaRoom.rpMetaList> metaList = metaService.metaList();
        // 5. 반환받은 List형태의 DTO를 바인딩한다.
        model.addAttribute("metaList", metaList);
        // 0. 검색에 사용할 DTO를 바인딩한다.
        model.addAttribute("metaDTO", new MetaRoom.rqSearchMetaList());
        // 메타 메인 페이지로 이동
    ...
    public String createMetaForm(Model model) {
        ...
        model.addAttribute("metaDTO", new MetaRoom.rqCreateMeta());
        // 방만들기 페이지로 이동
    ...
    public String createMeta(MetaRoom.rqCreateMeta rqCreateMeta) { // 1. DTO로 form값을 다 받아온다.
    ...
    // 방 이름 및 분야별로 검색
    @GetMapping("/search")
    public String searchMeta(MetaRoom.rqSearchMetaList rqSearchMetaList, Model model) { // 1. DTO로 form값을 다 받아온다.
        // 2. 받아온 DTO를 서비스에 넘겨준다.
        List<MetaRoom.rpSearchMetaList> rpSearchMetaList = metaService.searchMetaList(rqSearchMetaList);
    ...
    // 방 나가기
    @GetMapping("/exit")
    public String exitRoom(@RequestParam long metaIdx) { // 1. 파라미터로 입장한 방 번호를 받아온다.
        // 2. 받아온 방 번호를 서비스에 넘겨준다.
        metaService.exit(metaIdx);
        // 메타 메인 페이지로 이동
        return "redirect:/meta";
    }
}
