...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public String studyRoom(@RequestParam long idx,
                            @RequestParam(value = "metaMaster", required = false) String metaMaster,
                            Principal principal, Model model) { // 1. 파라미터로 입장한 방 번호를 받아온다.
                                                                //    URL 파라미터를 통해 방 번호와 같이 넘어오는 방 생성자 닉네임이 있을 경우 파라미터로 받아온다.
        ...
        if ( session.getAttribute(rpNickImage.getNickname()) != null ) {
            ...
            Meta.rpEntrance rpEntrance = metaService.reEntrance(idx, rpNickImage);
        ...
        // 4-2. 세션에 값이 존재하지 않는 경우 - 첫 입장
            ...
            // 6. 1에서 파라미터로 받아온 방 생성자 닉네임이 방 번호와 같이 넘어왔는지 체크한다.
            // 6-1. 방 생성자 닉네임이 넘어온 경우 - 방장 참여
            if ( metaMaster != null ) {
                // 6-1-1. 1에서 파라미터로 받아온 방 번호와 2에서 반환받은 DTO를 서비스에 전달한다.
                Meta.rpEntrance rpEntrance = metaService.masterEntrance(idx, rpNickImage);
                // 6-1-2. 1에서 파라미터로 받아온 방 번호를 서비스에 전달한다.
                List<MetaRoom.rpMetaRoomIdxList> rpMetaRoomIdxList = metaService.metaRoomParticipant(idx);
                // 6-1-3. 2에서 반환받은 로그인 유저 정보 DTO를 바인딩한다.
                model.addAttribute("nickImage", rpNickImage);
                // 6-1-4. 6-1-1에서 반환받은 입장한 방 정보 DTO를 바인딩한다.
                model.addAttribute("metaRoom", rpEntrance);
                // 6-1-5. 6-1-2에서 반환받은 입장한 방 내부 참여자 명단 DTO를 바인딩한다.
                model.addAttribute("participantList", rpMetaRoomIdxList);
                // 6-1-6. 스터디룸 페이지로 이동한다.
                return "Meta/StudyRoom";
            // 6-2. 방 생성자 닉네임이 넘어오지 않은 경우 - 유저 참여
            } else {
                // 7. 1에서 파라미터로 받아온 방 번호와 2에서 반환받은 DTO를 서비스에 전달한다.
                Meta.rpEntrance rpEntrance = metaService.newEntrance(idx, rpNickImage);
                // 8. 6에서 반환받은 DTO가 있는지 체크한다.
                // 8-1. 반환받은 DTO가 없는 경우 - 해당 방이 없는 경우
                if (rpEntrance == null) {
                    // 8-1-1. 에러메시지를 바인딩한다.
                    model.addAttribute("err", "해당 방의 정보가 없습니다.");
                    // 8-1-2. 메타 메인 페이지로 이동한다.
                    return "Meta/MetaRoom";
                // 8-2. 반환받은 DTO가 있는 경우 - 해당 방이 있는 경우
                } else {
                    ...
                    if (rpEntrance.getIdx() == 0) {
                        ...
                        // 9-1-2. 메타 메인 페이지로 이동한다.
                        return "Meta/MetaRoom";
                    // 9-2. idx가 0이 아닌 경우 - 해당 방에 입장
                    } else {
                        // 9-2-1. 1에서 파라미터로 받아온 방 번호를 서비스에 전달한다.
                        List<MetaRoom.rpMetaRoomIdxList> rpMetaRoomIdxList = metaService.metaRoomParticipant(idx);
                        // 9-2-2. 2에서 반환받은 로그인 유저 정보 DTO를 바인딩한다.
                        model.addAttribute("nickImage", rpNickImage);
                        // 9-2-3. 7에서 반환받은 입장한 방 정보 DTO를 바인딩한다.
                        model.addAttribute("metaRoom", rpEntrance);
                        // 9-2-4. 9-2-1에서 반환받은 입장한 방 내부 참여자 명단 DTO를 바인딩한다.
                        model.addAttribute("participantList", rpMetaRoomIdxList);
                        // 9-2-5. 스터디룸 페이지로 이동한다.
                        return "Meta/StudyRoom";
                    }
                }
            }
    ...
    public String exitRoom(@RequestParam long idx, String nickname) { // 1. 파라미터로 입장한 방 번호와 닉네임을 받아온다.
        ...
        HttpSession session = request.getSession();
        // 8. 7에서 가져온 세션 객체를 통해 1에서 파라미터로 받아온 닉네임에 해당하는 세션을 제거한다.
        session.removeAttribute(nickname);
        // 9. 메타 메인 페이지로 리다이렉트한다.
    ...
    public String deleteRoom(@RequestParam long idx, String nickname) { // 1. 파라미터로 입장한 방 번호와 닉네임을 받아온다.
        ...
        // 3. 세션을 사용하기 위하여 HttpServletRequest을 통해 세션 객체를 가져온다.
        HttpSession session = request.getSession();
        // 4. 3에서 가져온 세션 객체를 통해 1에서 파라미터로 받아온 닉네임에 해당하는 세션을 제거한다.
        session.removeAttribute(nickname);
